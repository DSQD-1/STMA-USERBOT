...
/*
 * users является особой таблицей.
 *
 * Новая схема требует:
 *
 * telegram_id INTEGER PRIMARY KEY
 *
 * Но старая таблица уже могла существовать с другим
 * PRIMARY KEY и вообще без telegram_id.
 *
 * Поэтому обычный ALTER TABLE здесь ненадёжен.
 *
 * Мы создаём новую таблицу и переносим максимально
 * возможное количество старых данных.
 */
...
async function migrateUsers() {
  ...
  console.log(
    "[DB MIGRATION] Rebuilding users table."
  );

  /*
   * Определяем возможное старое поле ID.
   */
  ...
  /*
   * Если старый ID неизвестен, данные всё равно
   * сохраняем в отдельную legacy-таблицу.
   */
  ...
  if (oldId) {
    ...
    /*
     * Сохраняем только реальные Telegram ID.
     *
     * SQLite CAST позволяет отсеять большую часть
     * мусорных значений.
     */
    ...
    console.log(
      `[DB MIGRATION] Migrated users using ${oldId}.`
    );
  } else {
    console.log(
      "[DB MIGRATION] No Telegram ID column found in legacy users table."
    );

    /*
     * Старые строки без Telegram ID нельзя корректно
     * привязать к Telegram-пользователю.
     *
     * Мы не переносим их в рабочую users.
     */
  }

  /*
   * Старую таблицу сохраняем как backup.
   */

  const backupExists = await tableExists(
    "users_legacy_backup"
  );
  ...
  if (!backupExists) {
    ...
    /*
     * Если backup уже существует, старую таблицу
     * просто удаляем после успешного создания новой.
     */
  ...
  console.log(
    "[DB MIGRATION] users table rebuilt successfully."
  );

  console.log(
    "[DB MIGRATION] Old users preserved as users_legacy_backup."
  );
}
...
async function migrateBusinessConnections() {
  ...
  /*
   * Если owner отсутствует, пробуем взять его
   * из user_id.
   */
...
export async function initDatabase() {
  ...
  /*
   * users обрабатываем ПЕРВЫМ,
   * потому что это таблица, которая сейчас реально
   * несовместима со старой схемой.
   */
