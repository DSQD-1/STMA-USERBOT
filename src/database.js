...
async function saveMessageEdit(message, connectionId) {
  const sender = message.from || {};
...
async function saveDeletedMessages(deleted, connectionId) {
  const messageIds =
    deleted.message_ids || [];
...
async function getIgnoredUsers(connectionId) {
  const result = await db.execute({
    sql: `
      SELECT user_id
      FROM ignored_users
...
async function setWatchSettings(
  ...
  await db.execute({
    ...
    args: [
      String(connectionId),
      merged.enabled ? 1 : 0,
      merged.new_messages ? 1 : 0,
      merged.edited_messages ? 1 : 0,
      merged.deleted_messages ? 1 : 0,
...
async function getWatchSettings(connectionId) {
  const result = await db.execute({
    sql: `
      SELECT *
      FROM watch_settings
  ...
  return {
    ...
    new_messages: Boolean(row.new_messages),
    edited_messages: Boolean(
      row.edited_messages
    ),
    deleted_messages: Boolean(
